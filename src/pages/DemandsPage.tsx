...
import { Copy, Clock, AlertTriangle, Filter, MessageSquare, CheckCircle2, X, ArrowUpDown, ArrowUp, ArrowDown } from "lucide-react";
...
export default function DemandsPage() {
  ...
  const responsaveis = useMemo(() => {
    const ids = new Set(mockTickets.flatMap((t) => t.assignees));
  ...
  }, [activeStatus, activePriority, activeCategory, activeResponsavel, showOverdue, sortField, sortDir]);

  const stats = useMemo(() => {
    const open = mockTickets.filter((t) => !["concluida", "cancelada"].includes(t.status));
    return {
                ...
                {sortedTickets.map((ticket) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5">
                        <PersonCell name={ticket.assigneeNames[0]} userId={ticket.assignees[0]} />
                      </td>
                      {!isShowingCompleted && (
                        <td className="px-3 py-2.5">
